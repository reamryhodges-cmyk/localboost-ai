...
export async function onRequestGet({ request, env }) {
  try {
    ...
    // Check logged-in LocalBoost customer
    ...
    // Google Ads connection is for paid customers
    const plan = String(user.plan || "").toLowerCase();

    if (!PAID_PLANS.includes(plan)) {
      return redirect(
        "/dashboard.html?google=paid_plan_required"
      );
    }
    ...
    // Remove expired/previous OAuth states
    ...
    return json(
      {
        success: false,
        error:
          "Could not start the Google Ads connection."
      },
      500
    );
  }
