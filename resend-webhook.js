...
export async function onRequestPost({ request, env }) {
  try {
    ...
    /*
      Resend webhooks use at-least-once
      delivery, so the same event can
    ...
    */
    ...
    /*
      ...
      svix_id has a UNIQUE index, so a
      duplicate webhook cannot create
    ...
    */
    ...
    try {
      ...
      /*
        If Resend retried at exactly the
        same moment and the UNIQUE index
        caught it, acknowledge the event.
      */
    ...
    /*
      Permanently block addresses that
      bounce, complain, fail or are
      suppressed by Resend.
    */
    ...
    /*
      Return 500 for an unexpected
      processing failure so Resend can
      retry delivery.
    */
...
async function verifySvixSignature({
  ...
  for (
    const signature of suppliedSignatures
  ) {
    const parts =
      signature.split(",");
